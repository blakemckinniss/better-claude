#!/usr/bin/env python3
"""Shared configuration for intelligence components across hooks."""
...
@dataclass
class SharedIntelligenceConfig:
    """Main configuration for shared intelligence components."""
    ...
    @classmethod
    def load_from_file(cls, config_path: Optional[Path] = None) -> "SharedIntelligenceConfig":
        """Load configuration from JSON file."""
        if config_path is None:
            # Look for config in standard locations
            search_paths = [
                Path.cwd() / ".claude" / "hooks" / "config" / "shared_intelligence.json",
                Path.cwd() / ".claude" / "hooks" / "config" / "pretooluse.json",  # Backward compatibility
...
_config: Optional[SharedIntelligenceConfig] = None


def get_config() -> SharedIntelligenceConfig:
    """Get the global configuration instance."""
    global _config
    if _config is None:
        _config = SharedIntelligenceConfig.load_from_file()
    return _config


def reload_config() -> SharedIntelligenceConfig:
