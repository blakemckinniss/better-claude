...
try:
    from hook_logger import HookLogger, logger
except ImportError:
...
class HookLoggerIntegration:
    ...
    @staticmethod
    def log_hook_entry(event_data: Dict[str, Any], hook_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def log_decision(event_data: Dict[str, Any], decision: str,
                    reason: Optional[str] = None, level: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if level is None:
                if HookLogger:
                    ...
                    # Fallback levels when HookLogger is not available
                    if decision in ["block", "deny"]:
