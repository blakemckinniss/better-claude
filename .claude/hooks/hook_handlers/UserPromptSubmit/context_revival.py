    ...
    def analyze_prompt(self, prompt: str) -> Dict[str, Any]:
        ...
        if found_keywords:
            ...
            analysis["reasons"].append(
                f"Contains trigger keywords: {', '.join(found_keywords[:3])}",
            )
        ...
        error_indicators_found = [
            ind for ind in self.error_indicators if ind in prompt_lower
        ]
        if error_indicators_found:
            analysis["confidence"] += 0.3
            analysis["reasons"].append(
                f"Error-related: {', '.join(error_indicators_found[:2])}",
            )

        # Check for success indicators (moderate confidence)
        success_indicators_found = [
            ind for ind in self.success_indicators if ind in prompt_lower
        ]
        if success_indicators_found:
            analysis["confidence"] += 0.2
            analysis["reasons"].append(
                f"Success-related: {', '.join(success_indicators_found[:2])}",
            )
...
class ContextFormatter:
    ...
    def _format_single_context(
        ...
        prompt = (
            f"{context.user_prompt[:200]}..."
            if len(context.user_prompt) > 200
        ...
        context_data = (
            f"{context.context_data[:800]}..."
            if len(context.context_data) > 800
    ...
    def format_contexts(
        self,
        contexts: List[ContextEntryType],
        ...
        for i, context in enumerate(sorted_contexts):
            ...
            context_section = self._format_single_context(
                context,
                i + 1,
                len(sorted_contexts),
            )
...
class ContextRevivalHook:
    ...
    def extract_file_context(self, prompt: str) -> List[str]:
        ...
        file_patterns = [
            ...
            r"[\w\-\./]+\.py|\.js|\.ts|\.jsx|\.tsx|\.go|\.rs|\.java",  # extensions
        ]
        ...
        for file in files:
            if any(
                file.endswith(ext) for ext in self.config["triggers"]["file_extensions"]
            ):
        ...
        try:
            ...
            try:
                ...
                if meaningful_words:
                    ...
                    for search_query in queries_to_try:
                        if search_query and search_query not in [
                            ...
                            for q in queries_to_try[
                                : queries_to_try.index(search_query)
                            ]
                        ]:
                            self.logger.debug(f"Trying search query: '{search_query}'")
                            try:
                                query_contexts = (
                                    self.context_manager.retrieve_relevant_contexts(
                                        search_query,
                                        files_involved,
                                ...
                                if query_contexts:
                                    self.logger.debug(
                                        f"Query '{search_query}' found "
                                        f"{len(query_contexts)} contexts",
                                    )
                    ...
                    relevance_threshold = self.config["retrieval"][
                        "relevance_threshold"
                    ]
                    contexts = [
                        ctx
            ...
            if elapsed > max_query_time:
                self.logger.warning(
                    f"Context retrieval took {elapsed:.2f}s "
                    f"(exceeds {max_query_time:.2f}s limit)",
                )
    ...
    def generate_context_injection(self, prompt: str) -> str:
        ...
        try:
            ...
            try:
                ...
                if injection:
                    self.logger.info(
                        f"Generated context injection with {len(contexts)} contexts",
                    )
                    return injection
                else:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_retrieval_time": (
                self.stats["total_retrieval_time"]
                / max(1, self.stats["contexts_retrieved"])
            ),
            "config": {
...
def get_context_revival_injection(
    prompt: str,
    project_dir: Optional[str] = None,
) -> str:
...
def apply_hook(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if context_injection:
            ...
            user_message["text"] = f"{context_injection}\n{prompt}"
