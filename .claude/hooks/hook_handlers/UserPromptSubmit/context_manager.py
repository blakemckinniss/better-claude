...
class ContextManager:
        ...
        try:
            ...
            return {
                "database": {
                    ...
                    "timeout": config.context.database_timeout,
                },
                "storage": {
                ...
                "circuit_breaker": {
                    "failure_threshold": 5,
                    "recovery_timeout": config.context.recovery_timeout,
                },
    ...
    def _init_database(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_contexts_relevance 
            ON contexts(relevance_score DESC);
    ...
    def __init__(
        ...
        try:
            ...
            self.context_dir = claude_dir / "hooks" / "context_revival"
            self.context_dir.mkdir(parents=True, exist_ok=True)
        except ValueError as e:
            raise ValueError(f"Failed to initialize context manager paths: {e}") from e
        ...
        self.circuit_breaker = CircuitBreaker(
            ...
            half_open_max_calls=self.config["circuit_breaker"].get(
                "half_open_max_calls",
                3,
            ),
        )
        ...
        # Cache
        self.cache: Dict[str, List[ContextEntry]] = {}
    ...
    def store_context(
        ...
        if self.config["storage"]["compression_enabled"] and len(context_data) > 1024:
        ...
        try:
            ...
            return ContextEntry(
                ...
                relevance_score=(
                    ...
                    if "relevance_score" in row.keys()
                    else 0.0
                ),
    ...
    def _calculate_relevance(
        ...
        weights = self.config["retrieval"].get(
            "scoring_weights",
            {
                "recency": 0.3,
                "relevance": 0.4,
    ...
    def _retrieve_contexts_db(
        ...
        with self.get_connection() as conn:
            ...
            cutoff_date = datetime.now() - timedelta(
                days=self.config["storage"]["max_context_age_days"],
            )
            ...
            for row in cursor:
                ...
                if entry:
                    ...
                    if (
                        ...
                        >= self.config["retrieval"]["relevance_threshold"]
                    ):
    ...
    def cleanup_old_contexts(self, days_old: Optional[int] = None) -> int:
        ...
        effective_days_old = (
            ...
            else self.config["storage"]["max_context_age_days"]
        )
        ...
        with self.get_connection() as conn:
            ...
            if row:
                return {
                    ...
                    "failed": row["failed"],
                    "success_rate": (row["successful"] / max(row["total_contexts"], 1))
                    * 100,
    ...
    def _update_context_outcome_db(
        self,
        context_id: int,
        outcome: str,
    ...
    def update_context_outcome(
        ...
        try:
            return self.circuit_breaker.call(
                self._update_context_outcome_db,
                context_id,
                outcome,
                metadata,
            )
